...
export default function Home() {
  ...
  return <main data-theme={theme}>
    <aside className="rail"><a className="monogram" href="#inicio" aria-label="Ir al inicio">ES<span>.</span></a><nav aria-label="Navegación principal">{["inicio", "experiencia", "formacion", "proyectos", "contacto"].map((item, index) => <a href={`#${item}`} key={item}><small>0{index + 1}</small><span>{item === "formacion" ? "formación" : item}</span></a>)}</nav><div className="rail-bottom">{now && <span className="rail-clock">{formatClock(now)}</span>}<Dot /> contacto abierto</div></aside>
    <div className="content">
      <section id="inicio" className="hero section-pad"><div className="hero-copy"><p className="eyebrow"><span>Ingeniería de proyectos</span> · Automatización & software</p><h1>Eduardo<br /><em>Schettino</em></h1><div className="hero-intro"><p>Perfil construido en dos etapas: Ingeniería en Automatización y Robótica, luego Ingeniería en Computación e Informática.</p><p>Enfocado en el aprendizaje continuo y la integración de tecnologías industriales y de software.</p></div><a className="round-link" href="#proyectos">Explorar proyectos <Arrow /></a></div><div className="portrait-wrap"><div className="portrait-label"><span>PERFIL</span><span>01 / 05</span></div><div className="portrait"><Image src="/IMG_6762.JPEG" alt="Eduardo Schettino" fill priority sizes="(min-width: 900px) 40vw, 90vw" /></div><div className="portrait-note"><Dot /> Abierto a contacto profesional</div></div><div className="hero-index">01</div></section>
      ...
      <section id="contacto" className="contact section-pad"><span className="section-index" aria-hidden="true">05</span><p className="eyebrow">05 · Contacto</p><div className="contact-grid"><h2>¿Hablamos de<br /><em>lo próximo?</em></h2><div><p>¿Tienes un proyecto de automatización, una consulta técnica o alguna idea de colaboración? Aquí están mis datos de contacto.</p>{contactOpen ? <div className="contact-data"><a href="mailto:e_schettino@icloud.com">e_schettino@icloud.com</a><a href="tel:+56988895490">+56 9 8889 5490</a></div> : <button className="contact-button" onClick={() => setContactOpen(true)}>Ver datos de contacto <Arrow /></button>}</div></div><footer><span>© {new Date().getFullYear()} Eduardo Schettino</span><span>Ingeniería de Proyectos</span></footer></section>
    </div>
